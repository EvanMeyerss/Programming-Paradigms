...
public class Model
{
	...
	private boolean eatPlayer = true;
	...
	private final int superPelletWidth = 34, superPelletHeight = 34;
	//---------------------------------------------
	...
	public void update() {
		...
		while(iter1.hasNext()) {
			...
			if (sprite1.isGhost()) {
				eatPlayer = ((Ghost)sprite1).eatPlayer();
			}
			...
			while (iter2.hasNext()) {
				...
				if (sprite1 != sprite2 && sprite1.doesItCollide(sprite2)) {
					...
					else if (sprite1.isPacman() && sprite2.isSuperPellet()) { // Pacman vs. Super Pellet
						((SuperPellet)sprite2).eatSuperPellet();
						Iterator<Sprite> iter3 = sprites.iterator();
						while(iter3.hasNext()) {
							Sprite sprite3 = iter3.next();
							if(sprite3.isGhost()) {
								((Ghost)sprite3).edible();
							}
						}
	...
	// Add fruit to grid
	public void setSuperPelletGridClick(int xClick, int yClick) {
		posXClick = (int)Math.floor(xClick / (float)gridWidth) * gridWidth;
		posYClick = (int)Math.floor(yClick / (float)gridHeight) * gridHeight;
		spriteAtCursorPos(posXClick, posYClick);		
		...
		if (!spriteFoundAtCursorPos) {
			// The "+5" in the coords centers the fruit
			Sprite sprite = new SuperPellet(posXClick + 8, posYClick + 8, superPelletWidth, superPelletHeight);
			queuedSprites.add(sprite);
		}
	}
// ------------------------------------------------------------------------------
// ------------------------------------------------------------------------------
    ...
    Json marshal()
    {
		...
		Json tmpListSuperPellets = Json.newList();
		ob.add("super_pellet", tmpListSuperPellets);
    ...
    public void unmarshal(Json ob)
    {
		...
		sprites.clear();
		// sprites.add(player);
		Json tmpListPacman = ob.get("pacman");
		for(int i = 0; i < tmpListPacman.size(); i++) {
			Sprite pac = new Pacman(tmpListPacman.get(i));
			sprites.add(pac);
			player = (Pacman)pac;
		}
		Json tmpListSuperPellet = ob.get("super_pellet");
		for(int i = 0; i < tmpListSuperPellet.size(); i++) {
			Sprite supPel = new Pacman(tmpListSuperPellet.get(i));
			sprites.add(supPel);
		}
