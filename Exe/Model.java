...
public class Model
{
	...
	public boolean isCollision(Wall wall) {		
		if (this.getPlayerX() > wall.getX() + wallWidth-1)
			...
			return false;
		if(this.getPlayerX() + playerWidth < wall.getX()+1)
			return false;
		// wallHeight-11 allows for the player's hair to slightly pass through an above block
		if(this.getPlayerY() > wall.getY() + wallHeight-1)
			return false;
		if(this.getPlayerY() + playerHeight < wall.getY()+1)
			return false;
		// Collision detected
		return true;
	}
