...
public class Pacman {
    private int x, y, prevX, prevY, w = 32, h = 40, direction, yFixed = 0, frameIteration = 1;
    ...
    public void getOutOfWall(Wall wall) {
        ...
        // Left
        if (x <= wall.getX() + wall.getWidth() && prevX >= wall.getX() + wall.getWidth()) {
            x = wall.getX() + wall.getWidth();
        }
        
        if ((y + h >= wall.getY() && y + h <= wall.getY() + wall.getHeight()) && prevY + h <= wall.getY()) {
            y = wall.getY() - h;
        }
        if ((y >= wall.getY() && y <= wall.getY() + wall.getHeight()) && prevY >= wall.getY() + wall.getHeight()) {
            ...
            y = wall.getY() + wall.getHeight();
                     ...
                     // if ((wall.getY() <= y && y <= wall.getY() + wall.getHeight()) && prevY >= wall.getY() + wall.getHeight());
